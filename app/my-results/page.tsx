...
function riskBadge(percent: number) {
  ...
  if (level === "lower") {
    return { label: "Lower Risk", cls: "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800" };
  }
  if (level === "moderate") {
    return { label: "Moderate Risk", cls: "bg-amber-50 dark:bg-amber-900/30 text-amber-900 dark:text-amber-300 border-amber-200 dark:border-amber-800" };
  }
  return { label: "Higher Risk", cls: "bg-red-50 dark:bg-red-900/30 text-red-800 dark:text-red-300 border-red-200 dark:border-red-800" };
}
    ...
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <aside className="fixed inset-y-0 left-0 flex w-64 flex-col border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 z-20">
        <div className="flex h-16 items-center px-6">
          <Link href="/my-results" className="text-xl font-bold tracking-tight text-sky-700 dark:text-sky-400">
            <Image src="/logo.png" alt="CKD One Logo" width={120} height={50} priority className="object-contain" />
          </Link>
        </div>
        <nav className="flex-1 space-y-1 px-3 py-4">
          <Link href="/my-results" className="flex items-center gap-3 rounded-md bg-sky-50 dark:bg-sky-900/30 px-3 py-2 text-sm font-medium text-sky-700 dark:text-sky-400">
          ...
          <div className="flex items-center justify-between rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-3">
             <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="flex flex-col truncate">
                   <span className="truncate text-sm font-semibold text-slate-900 dark:text-slate-50">{patientName || 'Patient'}</span>
                   <span className="truncate text-xs text-slate-500 dark:text-slate-400">Patient</span>
                </div>
             </div>
             <button
      ...
      <div className="flex flex-1 flex-col pl-64">
        ...
        <main className="flex-1 p-8">
           <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between mb-8">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Here is your screening history.</p>
              </div>
        ...
        {loading ? (
          ...
          /* empty state */
          <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-12 text-center shadow-sm">
            ...
            <h2 className="mt-4 text-lg font-semibold text-slate-900 dark:text-slate-50">No screenings yet</h2>
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">You haven't completed a screening yet.</p>
            <Link
              href="/screening"
            ...
            {screenings.map((p, i) => {
              ...
              return (
                <div
                  ...
                  className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm transition hover:border-sky-300 hover:shadow-md cursor-pointer group"
                >
                  <div>
                    <p className="text-sm font-semibold text-slate-900 dark:text-slate-50">{formatDate(p.created_at)}</p>
                    <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">View detailed report and explanation</p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {p.boundary_near ? (
                      <span 
                        ...
                        className="inline-flex items-center gap-1 rounded-full border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/30 px-2.5 py-0.5 text-xs font-semibold text-amber-900 dark:text-amber-300 cursor-help"
                      >
                        Near Boundary
