...
function stabilityBadge(stability: string) {
  if (stability === "borderline") {
    return { label: "Borderline", cls: "bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-800" };
  }
  if (stability === "unstable") {
    return { label: "Unstable", cls: "bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 border-red-200 dark:border-red-800" };
  }
  return { label: "Stable", cls: "bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-800" };
}
    ...
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <aside className="fixed inset-y-0 left-0 flex w-64 flex-col border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 z-20">
        ...
        <nav className="flex-1 space-y-1 px-3 py-4">
          ...
          <Link href="/history" className="flex items-center gap-3 rounded-md bg-sky-50 dark:bg-sky-900/30 px-3 py-2 text-sm font-medium text-sky-700 dark:text-sky-400">
          ...
          <div className="flex items-center justify-between rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 p-3">
             <div className="flex items-center gap-3 overflow-hidden">
                ...
                <div className="flex flex-col truncate">
                   <span className="truncate text-sm font-semibold text-slate-900 dark:text-slate-50">{userName || (userRole === "doctor" ? "Doctor" : "Patient")}</span>
                   <span className="truncate text-xs text-slate-500 dark:text-slate-400">{userRole === "doctor" ? "Nephrologist" : "Patient"}</span>
                </div>
             </div>
      ...
      <div className="flex flex-1 flex-col pl-64">
        ...
        <header className="sticky top-0 z-10 flex h-16 shrink-0 items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-8">
          <div className="flex flex-1 items-center gap-4">
            <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-slate-50">History</h1>
          </div>
             ...
             <div className="max-w-6xl">
                    ...
                    {(["all", "lower", "moderate", "higher"] as RiskFilter[]).map((f) => (
                      <button
                        ...
                        className={`rounded-full px-4 py-1.5 text-xs font-medium transition ${
                          filter === f
                            ? "bg-slate-900 dark:bg-sky-600 text-white border border-transparent"
                            : "bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 shadow-sm"
                        }`}
                      >
               ...
               {sorted.length === 0 ? (
                 <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-12 text-center shadow-sm">
                   <p className="text-sm text-slate-500 dark:text-slate-400">No screenings found.</p>
                 </div>
               ) : (
